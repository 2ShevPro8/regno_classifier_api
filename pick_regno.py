...
def regno_category(regno):
    if re.fullmatch(r'^[А-Я]\d{3}[А-Я]{2}\d{2}(|\d)$', regno):
        return 'private car'
    elif re.fullmatch(r'^[А-Я]{2}\d{5}(|\d)$', regno):
        return ' lk_taxi_r_pricep'
    elif re.fullmatch(r'^[А-Я]{2}\d{6}(|\d)$', regno):
        return 'lk_pricep_r_transit'
    elif re.fullmatch(r'^\d{4}[А-Я]{2}\d{2}(|\d)$', regno):
        return 'lk_moto_tract'
    elif re.fullmatch(r'^[А-Я]{1}\d{4}[А-Я]{2}$', regno):
        return 'some'
    elif re.fullmatch(r'^[А-Я]{2}\d{3}[А-Я]\d{2}(|\d)$', regno):
        return 'lk_transit'
    elif re.fullmatch(r'^(T|Т)[А-Я]{2}\d{5}(|\d)$', regno):
        return 'lk_export'
    elif re.fullmatch(r'^[А-Я]\d{6}(|\d)$', regno):
        return 'lk_mvd_avto'
    elif re.fullmatch(r'^\d{4}[А-Я]\d{2}(|\d)$', regno):
        return 'lk_mvd_moto'
    elif re.fullmatch(r'^\d{3}[А-Я]\d{2}(|\d)$', regno):
        return 'lk_mvd_pricep'
    elif re.fullmatch(r'^\d{3}(CD|СD|D|T|Т)\d{3}(|\d)(|\d)(|\d)$', regno):
        return 'lk_diplomat'
    elif re.fullmatch(r'^\d{4}[А-Я]{2}\d{2}(|\d)$', regno):
        return 'lk_army'
    else:
...
def pick_regno(camera_regno, nn_regno, camera_score, nn_score, nn_sym_scores, nn_len_scores, camera_type, camera_class, time_check, direction, model):
